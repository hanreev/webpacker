...
export const webpackerCompiler = (
  ...
  if (argv.progress) new webpack.ProgressPlugin().apply(compiler);

  if (typeof outputOptions === 'boolean' || typeof outputOptions === 'string')
    outputOptions = (webpack.Stats as any).presetToOptions(outputOptions);
  else if (!outputOptions) outputOptions = {};

  if (argv.color && process.stdout.isTTY) (outputOptions as any).colors = require('supports-color').stdout;

  if (runCompiler) {
    const compilerCallback = (err: Error, stats: webpack.Stats) => {
      if (!argv.watch || err) (compiler as any).purgeInputFileSystem();
      ...
      if (argv.json) process.stdout.write(JSON.stringify(stats.toJson(outputOptions), null, 2) + '\n');
      ...
      if (!argv.watch && stats.hasErrors()) process.exitCode = 2;
    };

    if (argv.watch) {
      const watchOptions = config.watchOptions || {};
      compiler.watch(watchOptions, compilerCallback);
      if ((outputOptions as any).infoVerbosity !== 'none') console.log('\nwebpack is watching the files…\n');
